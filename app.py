...
def run_cycle():
    ...
    if not config.LIVE_TRADING:
        ...
        _touch_notification(db)
        # NUEVO (16/09/2026, pedido del usuario): decision_id vincula esta
        # posición con la fila de `decisions` que la originó, y stake_dollars
        # guarda el monto realmente arriesgado -- ambos se copian a
        # closed_trades al cerrar (ver close_trade_with_outcome). REAPLICADO
        # (16/09/2026): un commit paralelo (216146e) había revertido estos
        # kwargs en los 4 call sites de este archivo sin tocar
        # supabase_db.py/db.py (que sí conservaron el soporte) -- quedó a
        # medias, decision_id/stake_dollars volvían a salir NULL siempre.
...
def run_mlb_cycle():
    ...
    # AUDITORÍA (16/09/2026): ver get_todays_stopped_mlb_keys() en
    # supabase_db.py -- complementa el dedupe de arriba (que solo cubre
    # señales todavía abiertas) para no re-señalar la misma apuesta ya
    # cortada por stop en el día.
    todays_stopped_keys = db.get_todays_stopped_mlb_keys()
    ...
    for market_raw in markets_raw:
        ...
        if signal["game_pk"] in open_game_pks:
            ...
            continue
        # AUDITORÍA (16/09/2026): ver get_todays_stopped_mlb_keys() en
        # supabase_db.py -- evita re-señalar la MISMA dirección de un
        # partido que ya se cortó por stop hoy. Si direction cambió
        # (el modelo pasó a favorecer al otro equipo), se deja pasar.
        if (signal["game_pk"], signal["direction"]) in todays_stopped_keys:
            detail.append({"question": market["question"], "status": "misma_direccion_ya_cortada_por_stop_hoy"})
            continue
        ...
        try:
            ...
            # AUDITORÍA (16/09/2026): ver MLB_STOP_LOSS_ENABLED en config.py --
            # desactivado por default, el análisis con datos reales
            # (analyze_mlb_stop_losses.py) mostró que este stop cortaba ~80%
            # de señales que terminaban ganando. stop=None hace que
            # run_mlb_track_results ni siquiera chequee el book contra un
            # stop (mismo `if stop is not None:` que ya usa el flujo de
            # clima), la señal corre hasta la resolución real del partido.
            stop = (
...
def handle_update(update):
    ...
    if decision == "approved":
        ...
        # REAPLICADO (16/09/2026): un commit paralelo (216146e) había
        # revertido justo este bloque -- volvió a la versión vieja (elif +
        # log_decision al final, sin decision_id) sin tocar
        # supabase_db.py/db.py, que sí conservaron el soporte.
