...
// ────────────────────────────────────────────────────────────────────
// Helper para parsear timestamps - soporta tanto formato Unix (segundos)
// como ISO 8601 (string) para compatibilidad con la migración de Semana 3
// ────────────────────────────────────────────────────────────────────
...
[ "Mi probabilidad ",  "Qué probabilidad le calcula el modelo del bot a que un bucket de temperatura ocurra, en base a los pronósticos (NWS, METAR, TAF) de la estación. Se compara contra el precio de mer cado para ver si hay ventaja. "],
[ "EV (valor esperado) ",  "La ventaja que ve el bot entre su probabilidad estimada y el precio de mercado, antes de tomar la señal. EV +15% significa que el bot cree que el mercado está subvalorando ese res ultado en un 15%. "],
...
[ "Brier score ",  "Qué tan calibrada estuvo la probabilidad del modelo contra lo que realmente pasó, en cada señal de clima resuelta. 0 = predicciones perfectas, 0.25 = tan bueno como tirar una mone da, 1 = siempre confiado y siempre equivocado. "],
...
// ahora reutilizado dent ro de una tarjeta de carrusel.
...
if (!checks || checks.length === 0) return nu ll;
return (
...
const enrichedRows = rows.map((r) => {
  ...
  const tsSignaled = parseTimestamp(r.ts_signaled);
  const tsResolved = parseTimestamp(r.ts_resolved);
